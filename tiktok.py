# DONT SKID MY CUTE CODE THANKS <33
##                                                                              
##        ,--,                                                                  
##      ,--.'|                                      ,-.                         
##   ,--,  | :  ,---,                           ,--/ /|                         
...
try:
    ...
    input("error while importing modules, install the modules in requirements.txt")
...
class automator:
    ...
    def __init__(self):
        ...
        try:
            ...
            self.error(f"Error loading webdriver: {e}")
        self.status = {}
        self.sent = 0
        self.cooldowns = 0
        self.ratelimits = 0
    ...
    def captcha_check(self):
        while True:
            ...
            os.system(clear)
            print(ascii_text)
            print(f"{self.console_message('Error')} do the captcha lol")
            time.sleep(1)

    def console_message(self, status):
        colour = Fore.RED
        if status == "Success":
            colour = Fore.GREEN
    ...
    def update_text(self):
        options = f"""
{self.console_message("1")} Follower Bot {Fore.RED}{self.status["followers"]}
{self.console_message("2")} Like Video Bot {Fore.RED}{self.status["likes"]}
{self.console_message("3")} View Bot {Fore.RED}{self.status["views"]}
{self.console_message("4")} Share Bot {Fore.RED}{self.status["shares"]}
        """
    ...
    def update_title(self, bot, remaining, rl = False):
        if clear == "cls":
            os.system(clear)
            ctypes.windll.kernel32.SetConsoleTitleW(f"TikTok MT | Sent: {self.sent} | Cooldown: {remaining}s | Developed by @4bonkas on Github")
            print(ascii_text)
            print(self.console_message(self.sent) + f" Sent {bot}")
            rl_cooldown = "0"
            cooldown = "0"
            if rl:
                ...
                cooldown = remaining
            print(self.console_message(self.cooldowns) + f" Cooldowns {Fore.WHITE}[{Fore.RED}{cooldown}s{Fore.WHITE}]")
            print(self.console_message(self.ratelimits) + f" Ratelimits {Fore.WHITE}[{Fore.RED}{rl_cooldown}s{Fore.WHITE}]")

    def main(self):
        if clear == "cls":
            ctypes.windll.kernel32.SetConsoleTitleW("TikTok MT | Developed by @4bonkas on Github")
        self.driver.get("https://zefoy.com/")
        time.sleep(2)
        if "502 Bad Gateway" in self.driver.page_source:
            os.system(clear)
            print(ascii_text)
            input(f"{self.console_message('Error')} no vpns or proxies.")
            os._exit(0)
        self.captcha_check()
        self.status_check()
        self.start()
    ...
    def error(self, error):
        print(ascii_text)
        print(f"{self.console_message('Error')} {error}")
        time.sleep(5)
        os._exit(0)
    
    def start(self):
        os.system(clear)
        print(self.update_text())
        try:
        ...
        if option == 1:
            ...
            div = 2
            ver = "followers"
            username = str(input(f"\n{self.console_message('Console')} tt user: @"))
            print()
            ...
            return self.start()
        video_url = str(input(f"\n{self.console_message('Console')} video link: "))
        print()
        check = self.check_link(video_url)
        if not check:
            return self.error("THAT VIDEO DOESNT EXIST YOU LIAR")
